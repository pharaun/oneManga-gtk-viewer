...
###############################################################################
sub new {
    my ($class, $width, $height, $manga) = @_;
    defined $width or throw MyException::Gtk_Viewer(
	    error => 'No width arg');
    defined $height or throw MyException::Gtk_Viewer(
	    error => 'No height arg');
    defined $manga or throw MyException::Gtk_Viewer(
	    error => 'No manga arg');
...
sub _initalize {
    ...
    $root_vbox->pack_start(Gtk_Common->new()->init_menu_bar(), $FALSE, $FALSE, 0);
