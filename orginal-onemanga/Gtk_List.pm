...
###############################################################################
sub new {
    my ($class, $width, $height, $model) = @_;
    defined $width or throw MyException::Gtk_List(
	    error => 'No width arg');
    defined $height or throw MyException::Gtk_List(
	    error => 'No height arg');
    defined $model or throw MyException::Gtk_List(
	    error => 'No model arg');
...
sub init_menu_bar {
    ...
    return Gtk_Common->new()->init_menu_bar($view_menu);
}
...
sub init_table {
    ...
    $self->{_model}->add_tree_view($tree_view);
